 ...
 * ??: watermarked = ? ? logo + (1-?) ? original
 *        => original = (watermarked - ? ? logo) / (1-?)
 *
 * ?? mask ?? GeminiWatermarkTool assets/embedded_assets.hpp
 */

export type WatermarkSize = 48 | 96

// ??? Gemini ????? (bg_48_png / bg_96_png)????? alpha ??
const BG_48_BASE64 =
...
/**
 * ???????? GeminiWatermarkTool ?????
 * ???????top-left = (W - margin_right - logo_size, H - margin_bottom - logo_size)
 * ???96?margin 64, 48?margin 32?????????????????
 */
 ...
 * @param alphaMap  alpha ? (0~1)
 * @param x ??????? x
 * @param y ??????? y
 * @param logoValue logo ????Gemini ??? 255
 */
...
/**
 * ???? base64 ???? alpha mask??? GeminiWatermarkTool?
 * ?????? 48 ??? 96?? 48 ? 1024 ??????
 */
